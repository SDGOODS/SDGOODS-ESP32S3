...
| 目录 | 内容 |
|---|---|
| `skills/` | 8 个**跨平台** Skill，封装 `tools/` 脚本与项目坑位 |
| `agent/sdgoods-dev.md` | 领域 Agent 定义（内嵌 `AGENTS.md` 硬约束 + 绑定 8 个 Skill） |
...
| `sdgoods-crash-triage` | 定位间歇性崩溃/内存被写坏/刷完不启动（addr2line 定代码行 → 只读探针 → 复现率二分 → 控制组 → 改一处再测） | 含 LVGL 定时器链表探针、串口复现率骨架、读到 ROM banner 后彻底静默的分层排除法 |
...
| 文件 | 作用 |
|---|---|
| `catalog.json` | 工具包**机器可读清单**：8 Skill + Agent + MCP server（7 工具）+ 3 平台的落点 / 安装命令 / MCP 配置模板。开放平台「Agent 市场」与「复制 MCP 配置」按钮的**单一数据源**。 |
