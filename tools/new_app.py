...
----------------
    ...
    2. ★ 只要出现**新的中文文案**，就重跑字体工具，否则屏上是方框（tofu）：
       ...
       然后重新编译、烧录
    3. 改完界面用截屏链路自证（电脑端运行）：
           python3 tools/screenshot_recv.py -p /dev/cu.usbmodemXXXX -o out.png
       在设备上「顶部下滑 → 截屏」即可，不用拔线拍照

想要像素风图标（像「飞机」「贪吃蛇」那样）
------------------------------------------
    在 main/apps/ui_app_page.c 的 icon_map_for() 里，按你的按钮文字
    加一条返回 12x12 像素图的规则即可；不加就显示文字标签。
"""
...
TEMPLATE_TITLE = "我的应用"   # app_template.c 里预置的标题，会被替换成用户给的
...
def main():
    ...
    ap.add_argument("title", help="启动台按钮上显示的文字（中文会占用字体子集）")
    ...
    print(f"应用: {name}   按钮文字: {args.title}")
    if args.dry_run:
        print("  [dry-run] 会创建:")
    ...
    insert_before_marker(
        ...
        [f'{{ "{args.title}", ui_{name}_show, ui_{name}_poll }},'],
        "注册到启动台",
    )
...
完成。接下来：
  ...
  1. 编辑 {os.path.relpath(out_c, ROOT)}，把界面改成你要的样子
     ...
     · 顶部下滑菜单 / 退出 / 暂停 / 截屏 已由 ui_app_shell 自动接好
