...
固件里预置了几个 demo，用来直观展示这块屏「能做什么」。下面**不是硬件全量清单**（全量见下方「硬件能力」），而是挑几个代表性 demo，看代码怎么用硬件：

### DEMO · 硬件基础能力演示

| Demo | 演示的硬件能力 |
|---|---|
| 主页 / 应用启动台（`ui_home.c`） | 圆形 360×360 触摸屏、电容触摸滑动手势、开机动画 |
| 演示页（`ui_demo_page.c`） | 基础 UI 控件、双语文案、屏与触摸的综合调用 |
| 小鸟（`ui_flappy.c`） | 触摸控制 + 定时器游戏循环 + 音频播放 |
| 飞机大战（`ui_plane.c` + `plane_net.c`） | 触摸 / 陀螺仪操控，以及 **WiFi + BLE 双人蓝牙联机对战** |
| 串口一键截屏 | 连电脑即抓当前画面（调试链路） |

### 应用 · 参考代码（给 AI 当模板）

想写自己的应用？直接把下面几个文件给 AI 看，它就是现成的参考实现：

| 参考文件 | 适合参考什么 |
|---|---|
| `main/apps/app_template.c` | **最小可运行骨架**（用 `tools/new_app.py` 一键生成，改它就成新应用） |
| `main/apps/ui_flappy.c` | 简单游戏：触摸输入 + 定时刷新 + 绘制 |
| `main/apps/ui_plane.c` + `plane_net.c` | 完整游戏 + 双人蓝牙联机同步逻辑 |
| `main/apps/ui_demo_page.c` | 页面布局、控件与硬件调用的写法 |
