...
Send this to the AI (swap in the app you want):
...
git clone https://github.com/SDGOODS/SDGOODS-ESP32S3
cd SDGOODS-ESP32S3
python3 tools/new_app_project.py MyApp   # derive a standalone app project in one command
...
Just tell the AI what to change in plain words: "change the title to XX", "play a sound when I tap the screen", "add a button that switches color when pressed". The AI finds the right file (all UI lives in `main/ui_<name>.c`), edits it and rebuilds.

If the badge is connected to the computer, add "**flash it and take a screenshot so I can see**" — you'll see the real device result on your screen without touching the badge.
...
- UI and logic live in `main/ui_<name>.c` of the derived project: `ui_<name>_start` builds the UI, `ui_<name>_poll` runs every frame, touch goes through the platform callbacks (`sdgoods_app_on_tap` / `sdgoods_app_on_gesture`).
