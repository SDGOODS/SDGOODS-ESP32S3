...
```bash
python3 tools/check_env.py            # 人类可读 + 分步进度；退出码 0=可编译，1=有必需项缺失
python3 tools/check_env.py --json     # 结构化输出（进度走 stderr，stdout 仍是纯 JSON）
```

逐条处理输出：
- `python3` / `ESP-IDF` / `esptool` 任何一个 `XX`（必失败）且退出码为 `1` → **停下，先帮用户把环境补齐**，
  不要凭「应该能编译」继续。ESP-IDF 报「已安装但未激活」时，先 `source <IDF>/export.sh` 再编译；
  ⚠️ **解包安装的 ESP-IDF（目录里没有 `.git`）用 `export.sh` 会静默失效**，此时改用它：
...
## 0.2 长任务要给进度提示（AI 行为约定）

编译 / 烧录 / 检查环境 / 发布固件这类任务动辄几十秒到十几分钟。
**中间连续 30 秒没有任何输出，用户就会以为卡死**。所以：

**① 开场先报路线图**：`📋 本次共 N 步，预计 X 分钟`。

**② 每步开始、结束各播一条**，格式固定：

| 时机 | 格式 | 示例 |
|---|---|---|
| 开始 | `⏳ [k/N] <步骤> —— 预计 ~<时长>` | `⏳ [2/6] 编译固件 —— 增量约 50 秒，全新约 3–6 分钟` |
| 成功 | `✅ [k/N] <步骤> —— <关键结果>` | `✅ [2/6] 编译完成 —— 1,683,856 字节` |
| 失败 | `❌ [k/N] <步骤> —— <原因>；已停在 xx` | `❌ [4/6] 烧录失败 —— 端口被占用，未继续` |

常见场景的划分与预估（按需取用，编号不强制）：

| 场景 | 步骤 | 预估 |
|---|---|---|
| 检查环境 | 7 项逐项检查 | 3–10 秒（脚本自带 `[k/7]` 分步进度） |
| 装环境（国内） | 切镜像 → 拉 SDK → 下工具链 → 装 Python 依赖 → 放 LVGL 离线包 | 10–40 分钟（约 1.2 GB） |
| 编译 + 烧录 | 激活环境 → 编译 → 产物核对 → 烧录 → 等开机 → 验证 | 2–6 分钟 |
| 发布到平台 | 通道 → 编译 → 探测设备 → 能力检查 → 搜集信息 → 截图 → 打包校验 → 上传 | 3–8 分钟 |

**③ 超过 60 秒的步骤一律后台跑**，启动后立刻说一句「已开始，跑完我回报」。
⚠️ **不要把输出全 tail 掉**：编译至少要保留 `Project build complete` 与
`xxx.bin binary size ...` 这两行——它们是「真的编出来了」的硬证据。

**④ 三种常见「卡住」要主动说明**，别让用户干等：

- **编译停在下载**：首次构建会由组件管理器拉 LVGL（~98 MB，国内可能很慢）。
  可先解压 24 MB 离线包，见 [`docs/ENVIRONMENT.md`](docs/ENVIRONMENT.md) §6。
- **烧录停在 `Connecting...`**：端口常被浏览器里的 Web Serial / 串口监视页面占住
  （`Resource busy`）。请用户关掉那个页面，**不要** `kill` 浏览器。
- **configure 长时间无输出**：全新构建要删大量文件，可能撞上批量删除守卫。先说明在等守卫，再排查。

**⑤ 播报是单向输出，不算打断用户** —— 不得为了播报进度而改成向用户提问。
需要用户决策的提问次数仍按各流程自己的约定来（发布流程是「只在四处打断」）。
