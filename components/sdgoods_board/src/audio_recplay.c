...
static volatile bool       s_bgm_fade_req;  /* 请求渐出（由 audio_bgm_stop 设置） */
...
esp_err_t audio_bgm_start(audio_bgm_theme_t theme)
{
    spk_hw_init();
    if (!s_tx) {
        return ESP_FAIL;
...
{
    if (!s_ready && audio_recplay_init() != ESP_OK) {
        return ESP_FAIL;
    }
    audio_bgm_stop();   /* 避免与游戏 BGM 抢占同一 I2S 发送通道 */
    s_abort = false;
    s_pcm_n = 0;
