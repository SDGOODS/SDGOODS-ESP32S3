/*
 * gifdec.c —— LVGL 8.3.11 内置 GIF 解码器的**修改版**（本项目补丁，随源码分发）
 *
 * 原始来源: LVGL 8.3.11  managed_components/lvgl__lvgl/src/extra/libs/gif/gifdec.c
 *           上游为 lecram/gifdec (MIT) 的改写版本；LVGL 自身亦为 MIT。
 *
 * 本文件相对上游的改动（仅这一点，其余保持原样）:
 *   1. canvas / frame 缓冲改由 heap_caps_malloc(MALLOC_CAP_SPIRAM) 分配 ——
 *      360x360 的 GIF canvas 在 16bpp 下约 518KB，远超 LVGL 内部内存池。
 *   2. canvas 由 RGB888(4B/px) 改为 RGB565(2B/px)，减半 PSRAM 写带宽。
 *   3. gd_open_gif_data() 把 GIF 源数据从 flash 拷到 RAM（优先内部 SRAM，
 *      退化到 PSRAM）——LZW 逐字节读 flash 会与取指争总线，解码慢到 ~85ms/帧。
 *   4. gd_GIF 增加 data_owned 字段，关闭时释放拷贝。
 *
 * 该文件由 main/patches/apply_lvgl_patches.py 在 CMake configure 阶段自动
 * 覆盖到 managed_components/ 下；managed_components 不入库（组件管理器重新下载），
 * 所以补丁必须以这种方式固化，否则别人 clone 后编译出的固件开机动画会失败。
 */
#include "gifdec.h"
...
gd_open_gif_data(const void *data, size_t data_size)
{
    ...
    /* Copy the GIF source bytes out of flash into fast memory.
       The LZW decoder reads the source stream byte-by-byte; reading it
       directly from the flash XIP region contends with instruction fetch
       (also from flash) and slows decoding to ~85ms/frame, which cannot
       sustain 15fps. Copying to RAM/PSRAM removes that contention.
       Prefer internal SRAM (fastest, separate bus from flash); if that is
       not available fall back to PSRAM. On failure we keep reading straight
       from flash (slower but still works). */
    uint8_t *copy = heap_caps_malloc(data_size, MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
    if (copy) {
        memcpy(copy, data, data_size);
        gif_base.data = (const char *)copy;
        gif_base.data_owned = 1;
        printf("BOOT gif src copied -> INTERNAL SRAM (%u bytes)\n", (unsigned)data_size);
    } else {
        copy = heap_caps_malloc(data_size, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
        if (copy) {
            memcpy(copy, data, data_size);
            gif_base.data = (const char *)copy;
            gif_base.data_owned = 1;
            printf("BOOT gif src copied -> PSRAM (%u bytes)\n", (unsigned)data_size);
        } else {
            printf("BOOT gif src copy FAILED, reading from flash (slow)\n");
        }
    }
    return gif_open(&gif_base);
}
...
static gd_GIF * gif_open(gd_GIF * gif_base)
{
     ...
     * 360x360 GIF canvas needs ~518KB (LV_COLOR_DEPTH==16), far larger than
     * LVGL's default internal memory pool. Allocate it from PSRAM so the rest
     * of LVGL can keep using fast internal SRAM. */
    size_t gif_canvas_size;
#if LV_COLOR_DEPTH == 32
    gif_canvas_size = sizeof(gd_GIF) + 5 * width * height;
#elif LV_COLOR_DEPTH == 16
    /* RGB565 canvas (2 bytes/pixel) + 1-byte index frame => 3*w*h.
       RGB565 halves the PSRAM write traffic vs the old 3-byte (RGB+pad)
       canvas, which is the decode-speed bottleneck on this chip. */
    gif_canvas_size = sizeof(gd_GIF) + 3 * width * height;
#elif LV_COLOR_DEPTH == 8 || LV_COLOR_DEPTH == 1
    gif_canvas_size = sizeof(gd_GIF) + 3 * width * height;
#endif
    gif = heap_caps_malloc(gif_canvas_size, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    if (!gif) goto fail;
...
{
    f_gif_close(gif);
    if (gif->data_owned && gif->data) heap_caps_free((void *)gif->data);
    heap_caps_free(gif);
}
