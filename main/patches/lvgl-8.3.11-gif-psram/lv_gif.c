/*
 * lv_gif.c —— LVGL 8.3.11 GIF 控件（lv_gif）的**修改版**（本项目补丁，随源码分发）
 *
 * 原始来源: LVGL 8.3.11  managed_components/lvgl__lvgl/src/extra/libs/gif/lv_gif.c  (MIT)
 *
 * 相对上游的改动（2 处）:
 *   1. gd_open_gif_data(img_dsc->data) → gd_open_gif_data(img_dsc->data, img_dsc->data_size)
 *      配套 gifdec 的改动：解码器需要把 GIF 源数据从 flash 拷到 RAM，因此必须知道长度。
 *   2. imgdsc.header.cf: LV_IMG_CF_TRUE_COLOR_ALPHA → LV_IMG_CF_TRUE_COLOR
 *      配套 canvas 改成 RGB565（无 alpha 通道），否则颜色会被当成带 alpha 解析、整屏发花。
 *
 * ⚠️ 这两个文件和 gifdec.{c,h} 必须**成套**替换，只改一半会导致
 *    「too few arguments to function 'gd_open_gif_data'」编译错误或画面颜色错误。
 */
/**
 * @file lv_gifenc.c
 *
...
void lv_gif_set_src(lv_obj_t * obj, const void * src)
{
    ...
    if(lv_img_src_get_type(src) == LV_IMG_SRC_VARIABLE) {
        ...
        gifobj->gif = gd_open_gif_data(img_dsc->data, img_dsc->data_size);
    }
